...
export default {
    ...
    methods :{
        async print(){
            ...
            doc.setFontSize(16)
            doc.text(80, 25, 'Demande fourniture')
            doc.setFontSize(12)
            ...
            /** Info Demande **/
            doc.setFontSize(12)
            doc.text(10, 40, 'Demande fourniture N  : ')
            doc.text(10, 46, 'Structure : ')
            doc.text(10, 52, 'Poste Téléphonique : ')
            doc.setFontSize(10)
            doc.text(54, 40, (this.demande.demande_ID).toString())
            ...
            .then(
             ...
             for (let i = 0 ; i <this.demande.length ; i++ ){
                        objets.push([
                            this.demande[i].code_object,
                            this.demande[i].designation ,
                            this.demande[i].qty_demande
                        ])
            }
            doc.autoTable({
                theme:'grid',
                margin: { top: 60 },
                head: [[{ content: 'Liste des objets', colSpan: 3, styles: { halign: 'center' ,fillColor: [255, 191, 0] } }]],
                body :objets
                
            })
            /**save  */
            doc.save("Demande fourniture némuro "+this.ID+".pdf"); 
        }
    }
}
</script>